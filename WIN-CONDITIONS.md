...
evidence is marked UNVERIFIED with what would settle it, rather than guessed.
    ...
    Asset we will own: the mainnet proof-relay settlement path, Starkscan STRK20 prover to screened attestation to apply_actions to card settlement in one transaction. Client is built (src/server/prover/starkscan.ts). STARKSCAN_API_KEY is present in .env as of 2026-09-05 (value never logged). A GET with that key to the SN_MAIN prove relay returned 403 on /prove and 404 "Prove job not found" on a fake job id, so the key is accepted and the relay is registered; prove-scope is UNVERIFIED until a real submit, which would debit the daily budget. 2 of 8 Cairo classes are declared on mainnet.
    ...
    Our product performs that verb: partly. HOLD yes on mainnet (0x04c4bea05417ce1062adef39b3d3b300f831ec994bbb4166d6010c4838d49193, Accepted on L1). SPEND yes on Sepolia (0x1f815361cd9cb1b378f208c8def10dddf5452ead190cb199a1da37adf4fe5df, Accepted on L1), no on mainnet: the prover key is in .env, prove-scope still UNVERIFIED, and 6 of 8 classes are undeclared on mainnet. EARN yes on Sepolia (EarnVault 1:1 ERC-4626, 10 STRK lent in the same transaction as the settlement, no yield). SEND no, needs a second registered pool recipient.
