...
- **SDK route: buildable, on a release candidate.** `@starkware-libs/starknet-privacy-sdk@0.14.3-rc.5` ships `transfers.build().subaccounts(dappName)` returning a `SubAccountsBuilder`, whose `invoke(nonce, { calls, collectPolicy })` queues a `ComputeAndInvoke` against the sub-account anonymizer. `SubAccountAnonymizerABI` is exported from the package entry point. It needs the new `subAccountAnonymizerAddress` field in the `createPrivateTransfers` config: calling `subaccounts(...)` without it throws. Note `identify()` and `deployed()` are declared but **not yet implemented**, and this is an RC: fine for a team that controls its own accounts and can track the API, not something to promise an end-user product on. Signers (`Snip12CallSetSigner`, `Eip712CallSetSigner`) are not on this package anymore; they moved to `@starkware-libs/starknet-privacy-client/signers`.
- **Wallet API route: still nothing usable.** Spec v0.10.4-rc.1 defines `wallet_strk20ShadowAccountCommitment` / `shadow_account_invoke`, but Ready support for those methods is unverified. Do not tell a dapp to call shadow methods until a connected wallet's `supportedWalletApi` actually advertises >= 0.10.4. Neither `@starknet-io/types-js@0.10.3` (stable) nor starknet.js exposes a sub-account method today.
