...
**Last re-verified 2026-08-14** — `check_freshness.py` on this date: get-starknet `next` is 6.0.4; `packages/sub_account_anonymizer` was renamed to `packages/shadow_account_anonymizer`; Wallet API latest stable is still v0.10.3 with v0.10.4-rc.1 in flight. `packages/escrow` is still absent. Re-run that check before finalizing a plan.
...
| Privacy SDK package | `@starkware-libs/starknet-privacy-sdk` | GitHub npm registry (not npmjs); **Node ≥ 24** required. Version `0.14.3-rc.5` in the monorepo as of 2026-08-14. Consumer dapp does not pin this. |
| Privacy client signers | `@starkware-libs/starknet-privacy-client/signers` | `Snip12CallSetSigner` + `Eip712CallSetSigner` moved here from the SDK. SDK package.json has no `./signers` export. |
| **Privacy client package** (new) | `@starkware-libs/starknet-privacy-client` | v0.1.0: `createPrivacyClient`, `SdkWallet`, `resolveShadowAccounts`, `AvnuPaymaster`, `deriveViewingKey`. Early — do not pin. Viewing-key APIs are why a consumer dapp stays on the Wallet API. |
...
| Anonymizer reference examples | `packages/ekubo_swap_anonymizer`, `packages/vesu_lending_anonymizer`, `packages/shadow_account_anonymizer` in the monorepo | Verified paths. Ekubo single-hop swap; Vesu lending deposit/withdraw; shadow-account anonymizer. Skeletons to adapt, not drop-in templates. The team owns review + audit. SDK shadow-account API ships; Wallet API shadow methods are spec-only until wallets advertise >= 0.10.4 |
...
| get-starknet **v6.0.4** | https://github.com/starknet-io/get-starknet | Install explicitly (npm `next` tag): `@starknet-io/get-starknet-discovery@6.0.4`, `@starknet-io/get-starknet-wallet-standard@6.0.4` |
| types-js **v0.10.3** | https://www.npmjs.com/package/@starknet-io/types-js/v/0.10.3 | Type definitions matching the Wallet API spec |
| Privacy Wallet API spec **v0.10.3** | https://github.com/starkware-libs/starknet-specs/releases/tag/v0.10.3 | Wallet-facing; dapps don't implement it. Latest stable. v0.10.4-rc.1 adds shadow-account methods; `@starknet-io/types-js@0.10.4-beta.2` has the types. Capability checks for Phase 1 stay at >= 0.10.3; do not call shadow methods until a wallet advertises >= 0.10.4 |
...
| What | Expected | Check |
|---|---|---|
| Private shadow accounts — **Wallet API in wallets** | Spec v0.10.4-rc.1 and types-js 0.10.4-beta.2 define `wallet_strk20ShadowAccountCommitment` and `shadow_account_invoke`. Stable types-js is 0.10.3. Ready/Xverse support is unverified. SDK route remains buildable. | Confirm the connected wallet's `supportedWalletApi` is >= 0.10.4 before calling |
